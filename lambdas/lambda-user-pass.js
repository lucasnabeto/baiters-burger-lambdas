  ...
  try {
    ...
    if (!username || !password) {
      return {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          message: "Dados de login ausentes. Certifique-se de que o corpo da requisição contenha username e password.",
        }),
      };
    }
    ...
    if (error.name === "NotAuthorizedException") {
      ...
      return {
        statusCode: 500,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: `Ocorreu um erro inesperado: ${error.message}` }),
      };
    }
  }
};
